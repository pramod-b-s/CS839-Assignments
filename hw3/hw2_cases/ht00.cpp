...
void createhtables(pmem::obj::pool<root> pop, int initcapacity){
	...
	pmem::obj::transaction::run(pop, [&]{
		...
		VALGRIND_PMC_REGISTER_PMEM_MAPPING(&(r1->hashtable1), sizeof(r1->hashtable1));
		r1->hashtable1 = pmem::obj::make_persistent<hash_table>(initcapacity,0);
		VALGRIND_PMC_REMOVE_PMEM_MAPPING(&(r1->hashtable1), sizeof(r1->hashtable1));
	});
	pmem::obj::transaction::run(pop, [&]{
		VALGRIND_PMC_REGISTER_PMEM_MAPPING(&((r1->hashtable)->entries), sizeof(vector_type));
		...
		VALGRIND_PMC_REMOVE_PMEM_MAPPING(&((r1->hashtable)->entries), sizeof(vector_type));
	});
	pmem::obj::transaction::run(pop, [&]{		
...
void put(pmem::obj::pool<root> pop){
	...
	pmem::obj::transaction::run(pop, [&]{
		...
		cnt=0;
		f=0;
		VALGRIND_PMC_REGISTER_PMEM_MAPPING(&(htable->entries), sizeof(vector_type));
		while(cnt<n && pvector[i].isval!=0){
			if(pvector[i].key==key1){
				auto t1 = pmem::obj::make_persistent<string_type>(t, strlen(t));
				pvector[i].value=t1;
		...
		VALGRIND_PMC_REMOVE_PMEM_MAPPING(&(htable->entries), sizeof(vector_type));
	});
}
...
void resize(pmem::obj::pool<root> pop){
	...
	pmem::obj::transaction::run(pop, [&]{
		...
		VALGRIND_PMC_REGISTER_PMEM_MAPPING(&(htable1->entries), sizeof(vector_type));
		...
		if(htnum==0){
			VALGRIND_PMC_REGISTER_PMEM_MAPPING(&(r->hashtable), sizeof(hash_table));
			pmem::obj::delete_persistent<hash_table>(r->hashtable); r->hashtable=htable1;
			VALGRIND_PMC_REMOVE_PMEM_MAPPING(&(r->hashtable), sizeof(hash_table));
		}
		else if(htnum==1){
			VALGRIND_PMC_REGISTER_PMEM_MAPPING(&(r->hashtable1), sizeof(hash_table));
			pmem::obj::delete_persistent<hash_table>(r->hashtable1); r->hashtable1=htable1;
			VALGRIND_PMC_REMOVE_PMEM_MAPPING(&(r->hashtable1), sizeof(hash_table));
		}
	});
}
